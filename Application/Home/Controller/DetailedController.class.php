...
class DetailedController extends CommonController {
    ...
    public function dtrading(){
        ...
        $where = $last_day1.'<='.$tq.'order.buytime and  '.$last_day2.'>='.$tq.'order.buytime';
        //查询多条记录
        $count = M('order')->where($tq.'order.uid='.$uid.' and '.$where)->count();
        $pagecount = 50;
        ...
        $show = $page->show();
        $list = M('order')->where($tq.'order.uid='.$uid.' and '.$where)->order($tq.'order.oid desc' )->limit($page->firstRow.','.$page->listRows)->select();   
        //计算出一段时间的盈亏beg
        //echo M('order')->_sql();
 //p($list);
 
        //计算总收益
        $trading['money']=M('order')->where($tq.'order.uid='.$uid.' and '.$where)->sum('ploss');
        //end
        //总单数
        $sumcount=M('order')->where($tq.'order.uid='.$uid.' and '.$where)->count();
        $trading['count']=$sumcount;
		
        //总手数 
        $sumonumber=M('order')->where($tq.'order.uid='.$uid.' and '.$where)->sum('onumber');
...
*/
    ...
    public function addorder(){
        ...
        */
        ...
        $uid=$_SESSION['uid'];
		//手续费
		$pxxx=M('productinfo')->where(array('pid'=>$pid))->find();
		$fee =$pxxx['feeprice'];
		...
		if(empty($leixing)){
			...
			if($goods['ptitle']=='原油'){
				$p = M('cl')->order('time DESC')->find();
				$buyprice=$p['price'];
			}elseif($goods['ptitle']=='原油'){
				$p = M('cl')->order('time DESC')->find();
				$buyprice=$p['price'];
			}elseif($goods['ptitle']=='白银'){
				$p = M('ag')->order('time DESC')->find();
				$buyprice=$p['price'];
			}elseif($goods['ptitle']=='沪铜'){
				$p = M('cu')->order('time DESC')->find();
				$buyprice=$p['price'];
			}elseif($goods['ptitle']=='比特币'){
				$p = M('bt')->order('time DESC')->find();
				$buyprice=$p['shoupan'];
			}
			...
			if ($orderid) {
	            ...
	            $accoun->aid=$user['aid'];
	            $accoun->uid=$uid;
	            $accoun->balance=$user['balance']-$money-$fee;
	            $accoun->save();
	            ...
	            $journal['jincome'] = $money;                                    //收支金额【要予以删除】
	            $journal['number'] = $money;                                    //数量
	            $journal['remarks'] = $goods['ptitle'];                         //产品名称  
	            $journal['balance'] = $user['balance']-$money-$fee;            //账户余额  
	            $journal['jstate'] = 0;                                         //盈利还是亏损
	            $journal['jusername'] = $username;                              //用户名
	            $journal['jostyle'] = $data['ostyle'];                          //涨、跌
	            $journal['juprice'] = $buyprice;                             //单价
	            $journal['jfee'] = $fee;                                      //手续费
	            $journal['jbuyprice'] = $mygetpeice;                            //入仓价
	            $journal['jaccess'] = ($money+$fee)-($money+$fee)*2;          //支持金额，求负数
