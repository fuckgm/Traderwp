...
class IndexController extends Controller {
    public function index()
    {
		...
		//die;
		//产品信息展示
		$plist = $product->order('pid desc')->limit(20)->select();
		...
		foreach($order as $k => $v){
			...
			if($ostyle==0){
				if($cid==1){
					...
					$ploss = round(($btprice-$buyprice)*$onumber*$wave*$myat,2);		//盈亏资金
				}
			}else{
				if($cid==1){
					...
					$ploss = round(($buyprice-$btprice)*$onumber*$wave*$myat,2);		//盈亏资金
				}
			}
