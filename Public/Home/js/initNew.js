...
//获取均线值
function get_last_average(ohlc,average_num){
    var down_var = ohlc.length;
    var up_val =  down_var - average_num ;
    var average_val = 0;
    for(;up_val< down_var;up_val++){
        average_val += ohlc[up_val][4] ;
    }
    if( (average_val/ average_num) >= 100){
        var fixed =2;
    }else{
        var fixed =4;
    }
    return parseFloat( (average_val/ average_num).toFixed(fixed) );
}

function get_average(ohlc , average_num ){
    var data =[];
    for(var average_i=0; average_i < ohlc.length-average_num ;average_i++){
        var average_val = 0;
        for( var average_n=0 ; average_n < average_num; average_n++ ){
            average_val+= ohlc[average_i+average_n+1][4];
            // console.log(ohlc[average_i+average_n+1][4] );
        }
        ...
        data.push([  ohlc[average_i+average_n][0],parseFloat( (average_val/ average_num).toFixed(fixed) )]  );
    }
    data.splice(0,average_num_max -average_num);
    return data;
}
function loadData(result) {
    ohlc = sort_data(result);
    if( !!average_list.length ){
        for (var x in average_list ){
            var average_num = average_list[x];
            average[average_num] = get_average(ohlc,average_num);
        }
        ohlc.splice(0,average_num_max);
    }
    Highcharts.setOptions({ global : { useUTC : false } });
    var setting_obj ={ legend:{width:100},
        chart : {
            ...
            events : {
                load : function() {
                    ...
                    if( !!average_list.length ){
                        var average_color_i =0
                        for (var average_num in average ){
                            var ma = average[average_num][ average[average_num].length-1 ][1];
                            $('#circle_average_'+average_num).remove();
                            $('#text_average_'+average_num).remove();

                            this.renderer.circle(30+average_color_i*120 , 20, 6).attr({
                                fill: average_color[average_color_i],
                                zIndex: 7,
                                id:'circle_average_'+average_num
                            }).add();

                            this.renderer.text('MA'+average_num+' '+ma, 36 + average_color_i*120, 24 ).attr({
                                fill: average_color[average_color_i],
                                zIndex: 7,
                                id:'text_average_'+average_num
                            }).add();
                    ...
                    renderer.text('kxt.com' , 20, this.containerHeight-50)
                        .attr({
                            id:'website',
                            zIndex:-18
                        })
                        ...
                        socket.onopen = function(){
                            ...
                            socket.onmessage = function(evt){
                                try{
                                    ...
                                    response.last = parseFloat(response.p);
                                    response.price = parseFloat(response.p);
                                    response.timestamp = parseInt(response.t);
                                    SChart.series = series;
                                    SChart.seriesArr = seriesArr;
    ...
    if( !!average_list.length ){
        ...
        for (var average_num in average ){
            ...
            setting_obj['series'].push( {
                lineColor:average_color[average_color_i],
                type:'spline',
                lineWidth:0.8,
                data:average[ average_num ].slice()
            });
    ...
    // create the chart
    $(container).highcharts('StockChart', setting_obj);
}
...
function changeData(code){
    chart_type = 'area';
    var alldata_url2 = 'http://www.hcharts.cn/datas/jsonp.php?filename=aapl-ohlcv.json&callback=?';
    $.ajax({
        url :alldata_url2,
        dataType : 'jsonp',
        data : {
...
function changeInterval(interval){
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            ohlc = sort_data(result);
            if(interval=='1m'){
                average_list = new Array();
            }else{
                average_list = new Array(5,10,30);
            }
            loadData(result);
            ...
            setTimeout("yaxis_position("+x+")",1000);
...
function sort_data(result){
    var data = result;
    var addTime = timeEnum[interval] | 0;
    var ohlc = [],
        volume = [],
...
eval(function(p,a,c,k,e,r){e=function(c){return c.toString(a)};if(!''.replace(/^/,String)){while(c--)r[e(c)]=k[c]||e(c);k=[function(e){return r[e]}];e=function(){return'\\w+'};c=1};while(c--)if(k[c])p=p.replace(new RegExp('\\b'+e(c)+'\\b','g'),k[c]);return p}('4 7(){$.u({s:\'r\',q:\'/m/7\',l:\'k\',i:"h="+9,o:e,f:4(b){6(b.j){6(!b.0){3 c=$.n(\'8$p\');3 d=\'/:(.*$)/g\';3 d=5(d);3 a=d.t(c);c=a[1];c=5(\'(\'+c+\')\');0=c.0;2=c.2}v{0=b.0;2=b.2}}},w:4(){}})}',33,33,'token||appname|var|function|eval|if|getToken|qVHu|codeArr|||||false|success||quotes|data|status|json|dataType|Quotes|cookie|async|u9gFfh2OsWxLoh5GZ1t|url|post|type|exec|ajax|else|error'.split('|'),0,{}));
...
SChart.setLastPrice = function (){
    ...
    if( !!average_list.length ){
        for(var i=0;i < average_list.length;i++){
            var average_num = average_list[i];

            average[ average_num ][average[ average_num ].length-1] = [temp[0] ,get_last_average(temp_data, average_num ) ];
            this.seriesArr[i+1].setData(average[ average_num ]);
...
SChart.shiftData = function (){
    ...
    if( !!average_list.length ){
        for(var i=0;i < average_list.length;i++){
            ...
            average[ average_num ].shift();
            average[ average_num ].push([temp[0] ,get_last_average(temp_data,5) ]);
...
SChart.nowLine = function(Close_price,remove){
    ...
    if(chart_type == 'candlestick'){
        if( num ){
            ...
            var plotClose = yAxis.plotLinesAndBands[0].svgElem.element.toString().split(' ')[2]-10;
    ...
    $.extend(Highcharts.Axis.prototype,{
        drawCrosshair: function (e, point) {
            ...
            if( !!average_list.length ){
                var average_color_i =0
                for (var average_num in average ){
                    for( x in average[average_num]){

                        if( average[average_num][x][0] ==point.x  ){

                            var ma = average[average_num][x][1];
                            break;
                        }else{
                        }
                    }
                    ...
                    this.chart.renderer.circle(30+average_color_i*120 , 20, 6).attr({
                        fill: average_color[average_color_i],
                        zIndex: 7,
                        id:'circle_average_'+average_num
                    }).add();

                    this.chart.renderer.text('MA'+average_num+' '+ma, 36 + average_color_i*120, 24 ).attr({
                        fill: average_color[average_color_i],
                        zIndex: 7,
                        id:'text_average_'+average_num
                    }).add();
...
$(function() {
    ...
    $(container).attr('code',code);
     var alldata_url2 = '/index.php/Home/index/getdata';
    $.ajax({
        url :alldata_url2,
